...
create index on players (profile_id);
...
create index on lessons (pillar, required_tier);
...
create index on drills (pillar);
create index on drills using gin (skill_tags);
create index on drills using gin (rubric_points);
...
create index on drill_logs (player_id, performed_on desc);
...
create index on assessments (email);
...
create table video_submissions (
  ...
  sla_due_at        timestamptz generated always as (submitted_at + interval '72 hours') stored,
  reviewed_at       timestamptz,
  reviewer_id       uuid references profiles(id)
);
create index on video_submissions (status, submitted_at);
create index on video_submissions (player_id, submitted_at desc);
...
create table review_prescriptions (
  review_id  uuid not null references analysis_reviews(id) on delete cascade,
  drill_id   uuid not null references drills(id) on delete cascade,
  sort_order int not null default 0,
